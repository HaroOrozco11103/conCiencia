...
function inicializar_tiempo(){
    try{
        var spans = $(".contadorTiempo").find("span");
        ...
        if((segundos == 0 && minutos == 0) || finDelJuego == true){
            ...
            //var spans = document.querySelector(".contadorTiempo").querySelectorAll("span");
    ...
    for(let i=0; i<tamanio*2; i++){
        ...
        do{
            ...
            if(posicionesLlenas[random]!=2)
            {
                ...
                $('.contenedor').append('  <div id="'+ i +'c" class="carta" onclick="verificar(this)">'  +
...
function verificar(carta){
    if(finDelJuego == false){   
        ...
        if(parCartas.length == 0){
            ...
            //Que no seleccione las misma carta 2 veces en un turno
            if(valorCartas[0].id != valorCartas[1].id){
